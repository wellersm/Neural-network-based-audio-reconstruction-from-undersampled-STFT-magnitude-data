...
def comp_measurements(input):
    spec_subroutine = T.Spectrogram(n_fft = 128, hop_length = 128, power = 1)
...
class nn_model(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            nn.Flatten(),
            nn.Linear(in_features=4095, out_features=27000,
                    bias=True),
            nn.SELU(),
            nn.Linear(in_features=27000, out_features=27000,
                    bias=True),
            nn.SELU(),
            nn.Linear(in_features=27000, out_features=8000,
                    bias=True)
        )
...
for epoch in range(50):
    ...
    if epoch == 0 or test_loss[epoch] < opt_loss:
        ...
        torch.save(model, f"Results4000/model_phaseI.pt")
...
plt.legend()
plt.grid(True)
fig.savefig(f"Results4000/plt_loss_phaseI.png", bbox_inches="tight")
# ---------- ---------- ----------


# ----------- Training - Phase II ----------
# Load model and decrease learning rate
model = torch.load("Results4000/model_phaseI.pt")
...
torch.save(model, "Results4000/model_phaseII.pt")
epochs_phaseII = 0
...
for epoch in range(int(epochs_phaseI//2+1)):
    ...
    if test_loss[epoch] < opt_loss:
        ...
        torch.save(model, f"Results4000/model_phaseII.pt")
...
plt.legend()
plt.grid(True)
fig.savefig(f"Results4000/plt_loss_phaseII.png", bbox_inches="tight")
# ---------- ---------- ----------
...
for epoch in range(num_epochs):
    ...
    if test_loss[epoch] < opt_loss:
        ...
        torch.save(model, f"Results4000/final.pt")
        start_timeII = time.time()

    else:

        model = torch.load("Results4000/final.pt")
        start_timeII = time.time()
...
plt.legend()
plt.grid(True)
fig.savefig(f"Results4000/plt_loss_phaseIII.png", bbox_inches="tight")
# ---------- ---------- ----------
...
plt.legend()
plt.grid(True)
fig.savefig(f"Results4000/plt_loss.png", bbox_inches="tight")
...
with torch.no_grad():
    ...
    fig.savefig(f"Results4000/rec.png", bbox_inches="tight")
    ...
    fig.savefig(f"Results4000/meas.png", bbox_inches="tight")